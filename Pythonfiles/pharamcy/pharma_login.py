...
def login():
    if request.method == 'POST':
       ...
       cur.execute('SELECT * FROM PHARMA_SIGNUP1 WHERE PHARMA_MAIL_ID = % s AND PHARMA_PASSWORD = % s',
...
def PHARMA_SIGNUP():
    if request.method == 'POST':
       try: 
         PHARMA_SIGNUP_ID = request.json['PHARMA_SIGNUP_ID']  
         ...
         cur = mysql.connection.cursor()
         cur.execute('INSERT INTO PHARMA_SIGNUP1(PHARMA_SIGNUP_ID,PHARMA_ID,PHARMA_NAME,PHARMA_MAIL_ID,PHARMA_PHONE_NUMBER,PHARMA_PASSWORD,PHARMA_IP,PHARMA_HOSTNAME) VALUES (%s,%s,%s, %s, %s, %s, %s,%s)',(PHARMA_SIGNUP_ID,PHARMA_ID,PHARMA_NAME,PHARMA_MAIL_ID,PHARMA_PHONE_NUMBER,h.hexdigest(),IPAddr,hostname))
